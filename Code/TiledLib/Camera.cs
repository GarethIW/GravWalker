...
namespace TiledLib
{
    public class Camera
    {
        ...
        public void Update(float speed, Rectangle bounds)
        {
            ...
            CameraMatrix = Matrix.CreateTranslation(-(int)Position.X, -(int)Position.Y, 0) *  Matrix.CreateRotationZ(-Rotation) * Matrix.CreateTranslation(Width/2, Height-(bounds.Height/3),0);
